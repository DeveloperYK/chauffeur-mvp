...
export async function editBookingAction(formData: FormData): Promise<EditBookingActionResult> {
  ...
  const raw = {
    ...
    bookedByName: String(formData.get('bookedByName') ?? '').trim() || null,
    bookedByPhone: String(formData.get('bookedByPhone') ?? '').trim() || null,
    bookedByEmail: String(formData.get('bookedByEmail') ?? '').trim() || null,
