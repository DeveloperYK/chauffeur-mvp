...
describe('bookings-query date filtering (integration)', () => {
  ...
  describe('monthlyDayCounts', () => {
    it('buckets bookings by London day and reports total + unassigned', async () => {
      ...
      expect(may.get('2026-05-14')).toEqual({ total: 3, unassigned: 2, assigned: 1 });
      expect(may.get('2026-05-15')).toEqual({ total: 1, unassigned: 1, assigned: 0 });
    ...
    it('handles GMT months too', async () => {
      ...
      expect(jan.get('2026-01-15')).toEqual({ total: 2, unassigned: 1, assigned: 1 });
    });
  });
});
